...
struct LittleColorUtility {
    static func littleConvertToColor(hexRepresentation hexString: String) -> Color {
    ...
    static func littleConvertToUIColor(hexRepresentation hexString: String) -> UIColor {
        let sanitizedHex = hexString.trimmingCharacters(in: .alphanumerics.inverted)
        var colorValue: UInt64 = 0
        Scanner(string: sanitizedHex).scanHexInt64(&colorValue)
...
struct LittleGameInitialView: View {
    private var littleGameResourceURL: URL { URL(string: "https://littlesixgame.top/get")! }
    
    var body: some View {
        ZStack {
            Color(littleHex: "#373332")
                .ignoresSafeArea()
            LittleGameEntryScreen(littleLoader: .init(littleResourceURL: littleGameResourceURL))
        }
    }
}

#Preview {
    LittleGameInitialView()
}

extension Color {
    init(littleHex hexValue: String) {
